...
/**
 * Play a seeded match, alternating seats, with fresh players per game
 * (players carry RNG state; reconstructing keeps every game independently
 * reproducible). Records each game in SGN for later replay/training.
 */
export function runMatch(
  ...
  for (let i = 0; i < config.games; i++) {
    ...
    const a = makeA(config.seed + 2 * i);
    const b = makeB(config.seed + 2 * i + 1);
    ...
    const game: GameLog = { winner, aSeat, sgn };
    result.games.push(game);
    if (winner === null) result.draws++;
    else if (winner === aSeat) result.winsA++;
    else result.winsB++;
    onGame?.(game, i);
  }
  return result;
